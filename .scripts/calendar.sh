...
PID=$(ps -ef | grep 'dzen2 -tw calendar' | grep -v grep | awk '{print $2}')
if [ $PID ]; then
    kill $PID
else
    ...
    done) | dzen2 -tw 'calendar' -l 19 -p -fn 'Ubuntu Mono:size=10' -e 'onstart=uncollapse,scrollhome;button1=exit;' -bg $WHITE -w $WIDTH -x $X_POS -y $Y_POS
fi
